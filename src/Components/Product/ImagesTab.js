...
const ImagesTab = ({ values, setFieldValue, errors, updateId }) => {
  return (
    <>
      ...
      <FileUploadField errors={errors} name="size_chart_image_id" id="size_chart_image_id" title="SizeChart" type="file" values={values} setFieldValue={setFieldValue} updateId={updateId} helpertext={'Upload an image the size chart tailored for fashion products. A table format image is suggested for easy reference.'} />
    </>
  );
};
